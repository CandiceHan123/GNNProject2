...
from Causal_VAE_Smile import CausalVAE
import argparse
...
if not os.path.exists(
        'figs_vae_smile/'):  # 判断所在目录下是否有该文件名的文件�?        os.makedirs('./logitdata_{}_{}/train/'.format(sample_num, context_dim))
    os.makedirs('figs_vae_smile/')
...
writer = SummaryWriter('./tensorboard/smile')


logger = get_logger('./loggers/model_smile.log')
...
for epoch in range(10):
    ...
    for u, l in train_dataset:
        ...
        m = len(train_dataset)
        save_image(u[0], 'figs_vae_smile/reconstructed_image_true_{}.png'.format(epoch), normalize=True)

        save_image(reconstructed_image[0], 'figs_vae_smile/reconstructed_image_{}.png'.format(epoch), normalize=True)
