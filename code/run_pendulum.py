...
from Causal_VAE_Pendulum import CausalVAE
import argparse
...
writer = SummaryWriter('./tensorboard/pendulum')

logger = get_logger('./loggers/model_pendulum.log')
...
for epoch in range(args.epoch_max):
    ...
    for u, l in train_dataset:
        ...
        h_a = _h_A(dag_param, dag_param.size()[0])  # dag_param和dag_param第0维的大小
        ...
        m = len(train_dataset)
        save_image(u[0], 'figs_vae_pendulum/reconstructed_image_true_{}.png'.format(epoch), normalize=True)
        save_image(reconstructed_image[0], 'figs_vae_pendulum/reconstructed_image_{}.png'.format(epoch), normalize=True)

    if epoch % 1 == 0:
        # print(str(epoch)+' loss:'+str(total_loss/m)+' kl:'+str(total_kl/m)+' rec:'+str(total_rec/m)+'m:' + str(m))
        logger.info(
            'Epoch:[{}/{}]\t loss = {:.5f}\t kl = {:.5f}\t rec = {:.5f}\t'.format(epoch, args.epoch_max, total_loss / m,
                                                                                  total_kl / m, total_rec / m))
        writer.add_scalars('add scalars', {'total_loss': total_loss / m, 'kl': total_kl / m, 'rec': total_rec / m},
                           epoch)
