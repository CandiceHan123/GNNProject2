...
def gaussian_parameters(h, dim=-1):
    ...
    """
    m, h = torch.split(h, h.size(dim) // 2, dim=dim)  # m, h [batch_size, 16]



    v = F.softplus(h) + 1e-8
    m, v = m.reshape([m.size()[0], 4, 4]), v.reshape([h.size()[0], 4, 4])
    return m, v
...
def kl_normal(qm, qv, pm, pv):  # qm = mu, qv = sigma2 , pm = 0 , pv = 1
    """
    ...
    """
    element_wise = 0.5 * (
                torch.log(pv) - torch.log(qv) + qv / pv + (qm - pm).pow(2) / pv - 1)  # - log_sigma2 + sigma2 + mu2 - 1
    kl = element_wise.sum(-1)  # 求每一行的和
    # print("log var1", qv)
    return kl
...
def load_Beard_BASE_by_name(model, global_step):
    """
    ...
    """
    file_path = os.path.join('checkpoints_Beard_BASE',
                             model.name,
...
def load_Beard_CNN_by_name(model, global_step):
    """
    Load a model based on its name model.name and the checkpoint iteration step
    ...
    Args:
        ...
        global_step: int: (): Checkpoint iteration
    """
    file_path = os.path.join('checkpoints_Beard_CNN',
                             model.name,
                             'model-{:05d}.pt'.format(global_step))
    state = torch.load(file_path)
    model.load_state_dict(state)
...
def load_BASE_by_name(model, global_step):
    """
    Load a model based on its name model.name and the checkpoint iteration step
    ...
    Args:
        ...
        global_step: int: (): Checkpoint iteration
    """
    file_path = os.path.join('checkpoints_BASE',
                             model.name,
                             'model-{:05d}.pt'.format(global_step))
    state = torch.load(file_path)
    model.load_state_dict(state)
...
def load_SPN_by_name(model, global_step):
    """
    Load a model based on its name model.name and the checkpoint iteration step
    ...
    Args:
        ...
        global_step: int: (): Checkpoint iteration
    """
    file_path = os.path.join('checkpoints_SPN',
                             model.name,
                             'model-{:05d}.pt'.format(global_step))
    state = torch.load(file_path)
    model.load_state_dict(state)
...
def load_GNN_by_name(model, global_step):
    """
    Load a model based on its name model.name and the checkpoint iteration step
    ...
    Args:
        ...
        global_step: int: (): Checkpoint iteration
    """
    file_path = os.path.join('checkpoints_GNN',
                             model.name,
                             'model-{:05d}.pt'.format(global_step))
    state = torch.load(file_path)
    model.load_state_dict(state)
...
def load_GNNSPN_by_name(model, global_step):
    """
    Load a model based on its name model.name and the checkpoint iteration step
    ...
    Args:
        ...
        global_step: int: (): Checkpoint iteration
    """
    file_path = os.path.join('checkpoints_GNNSPN',
                             model.name,
                             'model-{:05d}.pt'.format(global_step))
    state = torch.load(file_path)
    model.load_state_dict(state)
...
# But do you tho ¯\_(�?_/¯
################################################################################
def save_Beard_BASE_by_name(model, global_step):
    save_dir = os.path.join('checkpoints_Beard_BASE', model.name)
...
def save_Beard_CNN_by_name(model, global_step):
    save_dir = os.path.join('checkpoints_Beard_CNN', model.name)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    file_path = os.path.join(save_dir, 'model-{:05d}.pt'.format(global_step))
    state = model.state_dict()
...
def save_BASE_by_name(model, global_step):
    save_dir = os.path.join('checkpoints_BASE', model.name)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    file_path = os.path.join(save_dir, 'model-{:05d}.pt'.format(global_step))
    state = model.state_dict()
...
def save_SPN_by_name(model, global_step):
...
def save_GNN_by_name(model, global_step):
    save_dir = os.path.join('checkpoints_GNN', model.name)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    file_path = os.path.join(save_dir, 'model-{:05d}.pt'.format(global_step))
    state = model.state_dict()
...
def save_GNNSPN_by_name(model, global_step):
    save_dir = os.path.join('checkpoints_GNNSPN', model.name)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    file_path = os.path.join(save_dir, 'model-{:05d}.pt'.format(global_step))
    state = model.state_dict()
...
def prepare_writer(model_name, overwrite_existing=False):
    ...
    save_dir = os.path.join('checkpoints_SPN', model_name)
