...
import Causal_VAE_Flow as sup_dag
import argparse
cuda = torch.cuda.is_available()
...
parser.add_argument('--dag',       type=str, default="sup_dag",     help="Flag for dag")
...
if args.dag == "sup_dag":
  ...
  ut.load_GNNSPN_by_name(lvae, 95)
  # ut.load_SPN_by_name(lvae, 100)
  # ut.load_GNN_by_name(lvae, 100)
  # ut.load_BASE_by_name(lvae, 100)
# mask 0 epoch
...
for u,l in train_dataset:
    ...
    for i in range(4):
        for j in range(-5, 5):
            re_label, re_dag, L, kl, rec, reconstructed_image, _ = lvae.negative_elbo_bound(u.to(device), l.to(device),
                                                                                            mask=i, sample=sample,
                                                                                            adj=j * 0)
        save_image(reconstructed_image[0], 'figs_test_vae_flow/reconstructed_image{}_{}.png'.format(i, count), range=(0, 1))
