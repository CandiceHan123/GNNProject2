...
import Causal_VAE_Pendulum as sup_dag
import argparse
cuda = torch.cuda.is_available()
...
if args.dag == "sup_dag":
  ...
  ut.load_SPN_by_name(lvae, 80)
  # ut.load_GNN_by_name(lvae, 80)
  # ut.load_SPN_by_name(lvae, 90)
  # ut.load_GNNSPN_by_name(lvae, 100)
...
train_dataset = get_batch_unin_dataset_withlabel(dataset_dir, 100, "train")
...
for u, l in train_dataset:
    ...
    for i in range(4):
        for j in range(-5, 5):
            re_label, re_dag, L, kl, rec, reconstructed_image, _ = lvae.negative_elbo_bound(u.to(device), l.to(device), mask=i, sample=sample, adj=0)
