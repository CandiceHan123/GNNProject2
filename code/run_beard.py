...
from Causal_VAE_Beard import CausalVAE
import argparse
...
parser.add_argument('--epoch_max', type=int, default=10, help="Number of training epochs")
parser.add_argument('--iter_save', type=int, default=2, help="Save model every n epochs")
parser.add_argument('--run', type=int, default=0, help="Run ID. In case you want to run replicates")
parser.add_argument('--train', type=int, default=1, help="Flag for training")
parser.add_argument('--color', type=int, default=False, help="Flag for color")
parser.add_argument('--toy', type=str, default="beard_mask", help="Flag for toy")
...
if not os.path.exists(
        'figs_vae_beard/'):  # 判断所在目录下是否有该文件名的文件�?        os.makedirs('./logitdata_{}_{}/train/'.format(sample_num, context_dim))
    os.makedirs('figs_vae_beard/')


dataset_dir = '../causal_data/causal_data/beard'
...
writer = SummaryWriter('./tensorboard/beard')


logger = get_logger('./loggers/model_beard.log')
...
for epoch in range(10):
    ...
    for u, l in train_dataset:
        ...
        m = len(train_dataset)
        save_image(u[0], 'figs_vae_beard/reconstructed_image_true_{}.png'.format(epoch), normalize=True)

        save_image(reconstructed_image[0], 'figs_vae_beard/reconstructed_image_{}.png'.format(epoch), normalize=True)
    ...
    if epoch % 1 == 0:
        ...
        # ut.save_Beard_BASE_by_name(lvae, epoch)
        ut.save_Beard_CNN_by_name(lvae, epoch)
