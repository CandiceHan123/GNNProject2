...
import Causal_VAE_Beard as sup_dag
import argparse
cuda = torch.cuda.is_available()
...
if args.dag == "sup_dag":
  ...
  # ut.load_Beard_BASE_by_name(lvae, 9)
  ut.load_Beard_CNN_by_name(lvae, 1)
  # ut.load_BASE_by_name(lvae, 6)
  # ut.load_GNN_by_name(lvae, 9)
  # ut.load_SPN_by_name(lvae, 100)
  # ut.load_GNNSPN_by_name(lvae, 6)
...
if not os.path.exists('./figs_test_vae_beard/'):
    os.makedirs('./figs_test_vae_beard/')
...
for u, l in train_dataset:
    ...
    for i in range(4):
        ...
        save_image(reconstructed_image[9], 'figs_test_vae_beard/reconstructed_image_{}_{}.png'.format(i, count), range=(0, 1))
    ...
    label_cnt += re_label
    dag_cnt += re_dag
    save_image(u[9], './figs_test_vae_beard/true_{}.png'.format(count))
    count += 1
    if count == 10:
        break
