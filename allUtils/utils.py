...
def matrix_poly(matrix, d):
    x = torch.eye(d).to(device) + torch.div(matrix.to(device), d).to(device)  # d*d的单位矩阵+A*A/d
    return torch.matrix_power(x, d)  # (I+A*A/d)^d


def _h_A(A, m):
    expm_A = matrix_poly(A * A, m)
    h_A = torch.trace(expm_A) - m  # trace矩阵的迹
    return h_A  # tr[(I+A*A/d)^d]-m
...
def get_batch_unin_dataset_withlabel(dataset_dir, batch_size, dataset):
