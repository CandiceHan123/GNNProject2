...
class CausalVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x, label, mask=None, sample=False, adj=None, alpha=0.3, beta=1, lambdav=0.001):
        ...
        # s1, s2, s3, s4 = q_m[:, 0, :], q_m[:, 1, :], q_m[:, 2, :], q_m[:, 3, :]
        #
        # sum1 = 0.25 * s1 * (s1 + s2 + s3 + s4)
        # sum2 = 0.25 * s2 * (s1 + s2 + s3 + s4)
        # sum3 = 0.25 * s3 * (s1 + s2 + s3 + s4)
        # sum4 = 0.25 * s4 * (s1 + s2 + s3 + s4)
        # q_m = torch.cat((sum1.reshape([-1, 1, 4]), sum2.reshape([-1, 1, 4]), sum3.reshape([-1, 1, 4]), sum4.reshape([-1, 1, 4])), dim=1)
        ...
        e3 = self.weight1 * e1 + self.weight2 * e2
        e4 = self.weight3 * e1 + self.weight4 * e2
        ...
        if sample is False:
            ...
            with torch.no_grad():
                z3 = self.weight1 * z1 + self.weight2 * z2
                z4 = self.weight3 * z1 + self.weight4 * z2
            z_given_dag = torch.cat((z1.reshape([-1, 1, 4]), z2.reshape([-1, 1, 4]), z3.reshape([-1, 1, 4]), z4.reshape([-1, 1, 4])), dim=1)

        # 解码过程 z->x
        decoded_bernoulli_logits, x1, x2, x3, x4 = self.dec.decode_sep(
            z_given_dag.reshape([z_given_dag.size()[0], 16]), label.to(device))
